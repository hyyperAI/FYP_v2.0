...
job_skills_selector = '.air3-token'  # multi - verified working selector from HTML analysis
...
def parse_one_job(driver: Chrome, job: Tag, index: int, fast: bool = False) -> dict[str, str | int | float | None]:
    ...
    try:
        ...
        job_title = job.select_one(job_title_selector).text[:50] if job.select_one(job_title_selector) else "Unknown"

        # Wait 20 seconds for captcha/page to fully load
        time.sleep(20)

        # Print: Clicking on job
        print(f"[CLICK] Clicking on job #{index}: {job_title}")
        print(f"       Using selector: article:nth-child({index})")

        # Click job link
        driver.execute_script("arguments[0].click();", driver.find_element(f"article:nth-child({index})"))

        # Wait 4 seconds for popup to fully load
        time.sleep(4)

        # Print: Extracting details
        print(f"[EXTRACT] Extracting details for: {job_title}")

        # Save page source for debugging
        with open(f"debug_job_{index}_page.html", "w", encoding="utf-8") as f:
            f.write(driver.page_source)
        print(f"[DEBUG] Saved page source to debug_job_{index}_page.html")

        job_soup = BeautifulSoup(driver.page_source, "html.parser")

        # Print: CSS selectors being used
        print(f"[SELECTORS] Using CSS selectors:")
        print(f"         - proposals: {proposals_selector}")
        print(f"         - client_location: {client_location_selector}")
        print(f"         - client_jobs_posted: {client_jobs_posted_selector}")
        print(f"         - client_hire_rate: {client_hire_rate_selector}")
        print(f"         - client_hourly_rate: {client_hourly_rate_selector}")
        print(f"         - client_total_spent: {client_spent_selector}")
        ...
        for key, selector, parse_func in zip(
            ...
            value = parse_func(element.text.split()[0]) if element else None
            job_details[key] = value
            print(f"         - {key}: {value}")

        # Print: Extracted values
        print(f"[VALUES] Extracted values:")
        print(f"         - proposals: {job_details['proposals']}")
        print(f"         - client_location: {job_details['client_location']}")
        print(f"         - client_jobs_posted: {job_details['client_jobs_posted']}")
        print(f"         - client_hire_rate: {job_details['client_hire_rate']}")
        print(f"         - client_hourly_rate: {job_details['client_hourly_rate']}")
        print(f"         - client_total_spent: {job_details['client_total_spent']}")

        # Wait 2 seconds to ensure extraction is complete
        time.sleep(2)

        # Print: Closing popup
        print(f"[CLOSE] Closing popup - clicking back button (<-) for job #{index}")

    except (SBNoSuchElementException, NoSuchElementException, TimeoutException):  # In case of a timeout or private job listing.
        print(f"[ERROR] Failed to scrape job #{index}: timeout or private listing")
        pass

    # Close popup using back arrow button (<-) before moving to next job
    try:
        driver.wait_for_selector(job_back_arrow_selector, timeout=5)
        driver.find_element(job_back_arrow_selector).click()
        print(f"[CLOSE] Popup closed successfully for job #{index}")
    except (SBWebDriverException, WebDriverException, TimeoutException):
        print(f"[ERROR] Failed to close popup for job #{index}")

    # Wait for popup animation to finish before next job
    time.sleep(0.5)
    return job_details
    ...
    def get_url_retry(self, driver: Chrome, page_number: int, msg: str | None = None) -> bool:
        ...
        for retry in range(self.retries):
            try:
                ...
                # Check if articles are present after waiting
                driver.find_element("css selector", "article")
            except NoSuchElementException:
                time_print(f"Encountered a Captcha or page not loaded for page {page_number}, trying again.")
                sleep(5, 15)
